...
class VisiOCR:
    ...
    @staticmethod
    def generate_visiting_pass(name, birth_date, age, phone, email, visited_time, expire_time):
        ...
        draw.text((50, 200), f"Phone Number: {phone}", font=font, fill=(0, 0, 0))
        draw.text((50, 250), f"Email: {email}", font=font, fill=(0, 0, 0))
        draw.text((50, 300), f"Visited Time: {visited_time}", font=font, fill=(0, 0, 0))
        draw.text((50, 350), f"Expire Time: {expire_time}", font=font, fill=(0, 0, 0))
...
class VisitorManagementSystem:
    ...
    def generate_pass(request):
        if request.method == 'POST' and 'image' in request.FILES:
            ...
            visited_time = request.POST.get('visited-time')
            expire_time = request.POST.get('expire-time')

            visiting_pass_image = VisiOCR.generate_visiting_pass(name, birth_date, age, phone, email, visited_time, expire_time)
            return render(request, 'home.html', {'name': name, 'birth_date': birth_date, 'age': age, 'phone': phone, 'email': email, 'visited_time': visited_time, 'expire_time': expire_time, 'visiting_pass_image': visiting_pass_image})  
